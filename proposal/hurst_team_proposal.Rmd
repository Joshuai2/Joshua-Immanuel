...
abstract: |
          | This project will involve looking at 30 years of individual NBA
          | player statistics. We will develop a number of new advanced analytics
          | and statistics to determine player efficient and value.
          | Using these numbers, we will rank players and determine
          | who should win prominent NBA awards. We will visualize these results
          | using bar graphs and sentiment analysis.
date: "March 15, 2019"
author:
  - Ajay Dugar
  - Eric Qian
...
- What problem or topic are you addressing?
  We are predicting are ask and bid price predicting.
  
- Why is it interesting or important? In particular, what evidence supports this   conclusion?
  Valueing assets are important. Being able to accurately price assets is one of the most fundamental skills in finance. Being able to do so is tremendously profitable. Many different subjects (math [Differential equations, Black-Scholes], physics [Brownian motion], engineering [signal processing], etc.) can be applied to finance, and this field is relatively young (<25 years old) and is growing. Many interesting and unsolved problems require statistical analysis, which is why we chose this project.
...
Prior research into this field and academic curiousity led us to this topic. We wanted to know if we coulduse a variety of variables to accurately predict future stock prices.

- What is your idea for addressing the problem or topic?
First, find and clean minute-by-minute data from the stocks making up an exchange-traded fund (ETF). Then train a linear model based on a variety of variables by minimizing MSE. Finally, test its predictive power on a series of stocks and visualize our results.
...
Our training set will be minute-by-minute data from the QTEC ETF, which comes from FactSet, a financial data service. Our testing data will be minute-by-minute data from IBM, which also comes from FactSet. The short time period for our data points will allow us for a greater degree of accuracy and precision in the evaluation of our model.

<https://finance.yahoo.com/quote/QTEC/options?p=QTEC&.tsrc=fin-srch>
<https://finance.yahoo.com/quote/IBM/options?p=IBM&.tsrc=fin-srch>
...
- What code will the group need to write for the project?
  1. Code for importing
  2. Code for cleaning the pre-collected data and randomizing it
  3. Code for training for a linear regression model where we choose the best model with the lowest MSE. In our case, we need to get the $\beta$ values for the model using the training data set(QTEC ETF).
  4. Code for evaluating the model by testing it against the test data set (IBM). 
  5. Code for creating the interactive user interface where the user can input what they want the program to do. 
  6. Code for the candle stick graph and making the graph interactive. 
...
It should be - while the data set is large, outside of the resoure requirement it should not be overly difficult to implement this specific idea. We will need to familiarize ourself with the specific data used here - lots of unique compiled stats are used in basketball as opposed to the typical counting stats. 
