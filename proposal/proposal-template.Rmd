...
It should be - while the data set is large, outside of the resoure requirement it should not be overly difficult to implement this specific idea.

- What steps must occur to complete the project before the end of the semester?
Accumulate the data in a timely manner - we must find what data we are going to use and apply our regression to it. Then test the predictive power by comparing it to an ETF and generate visualizations. Building the model/obtaining the data will probably the most time intensive portion fo the project.
...
- What is the work plan to accomplish the necessary tasks before the end of the
  ...
  - Consider making a [Gantt chart](https://en.wikipedia.org/wiki/Gantt_chart)
    ...
    Ajay       - find and clean data
    Joshua     - choose variables
    Eric       - build model
    Bhanu      - train model
    Ajay       - apply model to IBM data
    Eric       - analyze validity of model
    Bhanu/Joshua (anyone else)     - visualize results
